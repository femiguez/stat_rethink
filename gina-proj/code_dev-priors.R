# Gina
# develop priors for root depth relationship
# 10/7/2020
...
# used https://apps.automeris.io/wpd/
...
#--not converging
fm1 <- nls(rootdepth_cm ~ SSbgrp(dap, w.max, lt.e, ldt), data = ro)
#--this one works
fm1 <- nls(rootdepth_cm ~ SSbgf(dap, w.max, t.e, t.m), data = ro)

#--logistic--

fm2 <- nls(rootdepth_cm ~ SSlogis(dap, Asym, xmid, scal), data = ro)

#--I think the logistic, fit to only the data with the highest point, answers our questions better. 
...
# slope at midpoint = scal*asym/4 (?) This doesn't work out. 
#https://www.tjmahr.com/anatomy-of-a-logistic-growth-curve/
# so scal = slope*4/asym
3*4/139
10*153/4
# this doesn't work
n <- 10
fun_logis <- function(x, Asym, xmid, scal){
  y = Asym / (1 + exp(-k * (x - xmid)))
  return(y)
}
